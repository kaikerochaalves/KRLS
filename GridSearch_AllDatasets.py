...
l_nu = [0.01, 0.1, 0.5]
l_sigma = [0.01, 0.05, 0.1, 0.5, 1, 5, 10, 100]
...
columns = ['nu', 'sigma', 'RMSE', 'NDEI', 'MAE']
result = pd.DataFrame(columns = columns)
for nu in l_nu:
    for sigma in l_sigma:
                            
        print(f"nu = {nu} and sigma = {sigma}")
    
        # Initializing the model
        model = KRLS(nu = nu, sigma = sigma)
        # Train the model
        ...
        NewRow = pd.DataFrame([[nu, sigma, RMSE, NDEI, MAE]], columns = columns)
